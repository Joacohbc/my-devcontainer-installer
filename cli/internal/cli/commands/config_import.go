...
func newConfigImportCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:         "devcontainer-cli import — reads a YAML configuration file and writes it to devcontainer.config.json in the current directory.\n\nThis is the inverse of the 'export' command, allowing you to quickly bootstrap a project's devcontainer setup from a pre-existing YAML template or shared configuration file.\n\nScope: Active workspace\n\nExamples:\n  devcontainer-cli import config.yml\n  devcontainer-cli import template.yml --force\n  devcontainer-cli import setup.yml -y",
