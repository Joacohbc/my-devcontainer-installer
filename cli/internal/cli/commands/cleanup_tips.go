...
func cleanupInstructionLines(config *types.DevcontainerConfig) []string {
	...
	return []string{
		...
		cli("devcontainer-cli clean containers [name...] [--all]"),
		cli("devcontainer-cli clean images [ref...] [--all]"),
		"",
		console.Bold("Purge CLI-managed resources and stale entries:"),
		cli("devcontainer-cli clean [catalog|containers|images|ssh|networks|volumes|all]"),
