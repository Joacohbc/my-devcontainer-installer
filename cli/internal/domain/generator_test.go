...
// Zellij is always-on, so even a minimal config (no modules selected) must ship
// it in the base image.
func TestGenerateDockerfile_MinimalIncludesZellij(t *testing.T) {
	df := mustGenerateDockerfile(t, makeConfig())
	assertContainsStr(t, df, "releases/latest/download/zellij-${ZELLIJ_ARCH}.tar.gz", "minimal dockerfile")
