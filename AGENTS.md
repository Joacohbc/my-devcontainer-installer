...
| `config` | `config.go` | Read/write global config (subcommands `registry`, `preset`); `config preset` (`preset.go`) lists/creates/copies reusable module-bundle presets saved under `~/.devcontainer-cli/presets/` |
