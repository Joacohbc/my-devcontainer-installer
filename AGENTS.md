...
| `ssh` | `ssh.go` | Open a real SSH session into a container (unlike `shell`'s `docker exec`); defaults to the project's own devcontainer, or any other managed container via `--container` (loose mode, same as `setup-ssh --container`) — no jump-host support, that's `port-forward`'s job. Auto-runs the `setup-ssh` flow first when no managed alias exists yet for the target (`SshService.ManagedAlias` looks it up by the `kind=workspace\|container` marker, not by assuming alias==name). `--forward`/`--ports` additionally open SSH tunnels alongside the session (`PortForwardService.OpenTunnelsDetached`, non-blocking), torn down when the session ends |
