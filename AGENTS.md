...
| `ssh` | `ssh.go` | Open a real SSH session into the project's own devcontainer (unlike `shell`'s `docker exec`); auto-runs the `setup-ssh` flow first when no managed alias exists yet for the workspace (`SshService.ManagedAlias` looks it up by the `kind=workspace` marker, not by assuming alias==workspace). `--forward`/`--ports` additionally open SSH tunnels alongside the session (`PortForwardService.OpenTunnelsDetached`, non-blocking), torn down when the session ends; no `--container` escape hatch — use `port-forward`/`shell -c` for other containers |
