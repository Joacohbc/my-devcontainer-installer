...
| `forward_ports:` | `ForwardPorts`, the tunnels `port-forward` opens with no argument | `port-forward` — `PORT`, `LOCAL:CONTAINER`, `LOCAL:SERVICE:CONTAINER`, each optionally prefixed `reverse:` |
...
A `forward_ports:` entry may also carry the direction, which `ports:` has no
equivalent for: `reverse:5432` is tunnelled host→container instead. The prefix
lives in the spec rather than in a flag because these are plain strings in a
manifest — there is nowhere else to put it, and the same prefix then works
everywhere a spec is accepted (`port-forward`, `--forward-ports`, `ssh --ports`,
the interactive picker's port prompt).
...
| `init` | `init.go` | Initialize a devcontainer environment: inspects workspace files to automatically detect technologies/stacks (Node.js/TS, Go, Python, Java/Maven/Gradle, PHP, Rust, C/C++, SQLite, GitHub Actions, Docker-in-Docker, Skills), binds them to modules, optionally clones a remote Git repo, generates `.dc_<ws>/`, builds and launches the stack, and installs skills if `skills-lock.json` is detected |
