...
var modeLabels = map[core.BuildMode]string{
	core.BuildModeLocalCached: "local-cached — generate Dockerfile + compose, reuse cached image when unchanged",
	core.BuildModeRemote:      "remote — skip the build, pull a pre-built image from the registry",
}

func choicesFromOption(o core.ModuleOption) []prompt.Choice {
...
func promptOption(o core.ModuleOption, def any) (any, error) {
	switch o.Type {
	case core.ModuleOptionSelect:
		s, _ := def.(string)
		return prompt.Select("  "+o.Label+":", choicesFromOption(o), s)
	case core.ModuleOptionMultiselect:
		return prompt.Multiselect("  "+o.Label+":", choicesFromOption(o), defaultStrings(def))
	case core.ModuleOptionConfirm:
		b := true
		if v, ok := def.(bool); ok {
			b = v
		}
		return prompt.Confirm("  "+o.Label+"?", b)
	case core.ModuleOptionInput:
		s, _ := def.(string)
...
func optionDefault(o core.ModuleOption, prevOpts map[string]any) any {
	if prevOpts != nil {
...
func buildConfigFromPrompts(base *core.DevcontainerConfig) (*core.DevcontainerConfig, error) {
	cwd, _ := os.Getwd()
	...
	modeChoices := make([]prompt.Choice, len(core.BuildModes))
	for i, m := range core.BuildModes {
		modeChoices[i] = prompt.Choice{Value: string(m), Label: modeLabels[m]}
	}
	modeDefault := string(base.Mode)
	if modeDefault == "" {
		modeDefault = string(core.BuildModeLocalCached)
	}
	modeStr, err := prompt.Select("Build mode:", modeChoices, modeDefault)
	if err != nil {
		return nil, err
	}
	mode := core.BuildMode(modeStr)

	var variant string
	if mode == core.BuildModeRemote {
		variantDefault := "ssh"
	...
	if mode == core.BuildModeLocalCached {
		...
		for _, id := range selectedIDs {
			...
			modules = append(modules, core.SelectedModule{ID: id, Options: opts})
		}
	}
	...
	for _, svc := range registry.ComposeServices {
		...
		services = append(services, core.SelectedModule{ID: svc.ID, Options: opts})
	}

	var selectableServices []prompt.Choice
	for _, s := range registry.ComposeServices {
	...
	for _, id := range serviceIDs {
		...
		services = append(services, core.SelectedModule{ID: id, Options: opts})
	}

	var image string
	if mode == core.BuildModeRemote && variant != "" {
		perProject := ""
		if base.Remote != nil {
	...
	draft := &core.DevcontainerConfig{
		Mode:       mode,
		Image:      image,
		Workspace:  workspace,
		Dockerfile: core.DockerfileConfig{Modules: modules},
		Compose:    core.ComposeConfig{Services: services, Subnet: subnet},
		Env:        env,
	}
	if mode == core.BuildModeRemote && variant != "" {
		perProject := ""
		...
		draft.Remote = &core.RemoteConfig{Variant: variant, Registry: perProject}
	}
...
func variantChoices() []prompt.Choice {
	choices := make([]prompt.Choice, len(core.RemoteVariants))
	for i, v := range core.RemoteVariants {
		choices[i] = prompt.Choice{Value: v, Label: core.VariantLabels[v]}
	}
	return choices
}
