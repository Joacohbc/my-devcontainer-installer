...
import (
	...
	"github.com/joacohbc/my-devcontainer-installer/cli-go/internal/infra/assets"
	...
	"github.com/joacohbc/my-devcontainer-installer/cli-go/internal/infra/sshinstructions"
	"github.com/joacohbc/my-devcontainer-installer/cli-go/internal/modules/compose"
...
func parseGenFlags(cmd *cobra.Command) (*genFlags, error) {
	...
	if g.mode != "" {
		if g.mode != string(core.BuildModeLocalCached) && g.mode != string(core.BuildModeRemote) {
			return nil, fmt.Errorf("invalid --mode: %s. Expected one of: %s", g.mode, strings.Join([]string{string(core.BuildModeLocalCached), string(core.BuildModeRemote)}, ", "))
		}
	}
	if g.variant != "" {
		if _, err := core.ParseVariant(g.variant); err != nil {
			return nil, err
		}
	}
	return g, nil
...
func runGenerate(cmd *cobra.Command, _ []string) error {
	...
	var config *core.DevcontainerConfig
	if existing != nil {
		config = existing
	} else {
	...
	} else if flags.interactive && flags.mode == string(core.BuildModeRemote) && flags.variant == "" &&
	...
	if config.Workspace == "" {
		config.Workspace = project.ResolveWorkspace(cwd, config)
	}
	if !domain.IsValidDockerName(config.Workspace) {
		return fmt.Errorf("invalid workspace name: %s", config.Workspace)
	}

	if config.Mode == core.BuildModeRemote && config.Image == "" && config.Remote != nil && config.Remote.Variant != "" {
	...
	if config.Mode == core.BuildModeRemote && config.Remote == nil {
		return fmt.Errorf("mode=remote requires --variant (one of: %s)", strings.Join(core.RemoteVariants, ", "))
	}
	...
	skipBuildArtifacts := config.Mode == core.BuildModeRemote
	...
	if config.Mode == core.BuildModeLocalCached && dockerfileContent != "" {
	...
	isRemote := config.Mode == core.BuildModeRemote
	build := flags.build
...
func applyGenFlags(config *core.DevcontainerConfig, flags *genFlags) {
	if flags.mode != "" {
		config.Mode = core.BuildMode(flags.mode)
	}
	if flags.workspace != "" {
		config.Workspace = flags.workspace
	}
	if flags.withModules != nil {
		var modules []core.SelectedModule
		for _, id := range flags.withModules {
			...
			if !found {
				modules = append(modules, core.SelectedModule{ID: id, Options: map[string]any{}})
			}
		}
	...
	if flags.services != nil {
		...
		for _, id := range flags.services {
			services = append(services, core.SelectedModule{ID: id, Options: serviceOptionsOf(config.Compose.Services, id)})
		}
		config.Compose.Services = services
	}
	if flags.mode == string(core.BuildModeRemote) || config.Mode == core.BuildModeRemote {
		...
		config.Remote = &core.RemoteConfig{Variant: variant, Registry: perProject}
	}
	if flags.image != "" {
		config.Image = flags.image
	} else if config.Mode == core.BuildModeRemote && config.Remote != nil {
		config.Image = domain.ResolveRemoteImage(config.Remote.Variant, domain.ResolveRegistry(flags.registry, config.Remote.Registry))
	}
}
